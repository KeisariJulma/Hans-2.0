...
class VoiceState:
    ...
    async def audio_player_task(self):
        while True:
            ...
            try:
                channel = self.bot.get_channel(int(config[str(self._ctx.guild.id)]['music_channel']))
            except KeyError:
                channel = self.bot.get_channel(self._ctx.channel.id)
            await channel.send(embed=self.current.create_embed())
            del channel
...
class Music(commands.Cog):
    ...
    @commands.command(name='leave', aliases=['disconnect', 'painu_vittuu'])
